...
class Expr {
   public:
      ...
      //constructors
      Expr(std::string input_expr):expr(input_expr){
        start = 0;
        end = expr.end()-expr.begin();
        this->Set();
      }; //root expression
      Expr(std::string input_expr, int start, int end): // bracket sub-expression
      expr(input_expr),start(start), end(end){
        this->Set();
      };
      Expr(std::string input_expr, int start, int end, bool is_term):
      start(start), end(end){
        if(is_term)
        expr = input_expr;
        else
        ...
        for(std::string::iterator it = expr.begin(); it!=expr.end(); it++){
          switch (*it){
            ...
            if(cur_b_lvl == 0){
              if(is_term){
                ...
                term_end = it;
                terms.push_back(Expr(std::string(term_start,term_end),term_start-expr.begin(), term_end-expr.begin(),true));
                is_term = false;
              }
              terms.push_back(Expr( std::string(1,*it),it-expr.begin(), it-expr.begin()+1,false));
            }
            break;
            case '(':
          ...
          if((*it)==')'){
            ...
            if(cur_b_lvl ==1){
              ...
              terms.push_back(Expr(std::string(start_loc,end_loc),start_loc-expr.begin()-1, end_loc-expr.begin()+1));
            }
            cur_b_lvl--;
          }
      ...
      void search2(){
        ...
        std::string term_id = "()";
        for(std::vector<Expr>::iterator it = terms.begin(); it!= terms.end(); it++){
        ...
        //group terms
        for(std::vector<Expr>::iterator it = terms.begin(); it <terms.end()-1; it++){
      ...
      void search3(){
        ...
        for(std::vector<Expr>::iterator it = terms.begin(); it< terms.end()-1; it++){
          ...
          else if(op_id1!= std::string::npos && op_id2!=std::string::npos && it< terms.end()-2){
            //this is an operator
      ...
      void search4(std::string op){
        ...
        std::size_t op_id;
        for(std::vector<Expr>::iterator it = terms.begin()+1; it< terms.end()-1; it++){
          op_id = op.find(it->fn);
...
void disp_content(Expr expr){
  ...
  if (expr.terms.empty()){
    ...
    for (std::vector<Expr>::iterator it = expr.terms.begin(); it!=expr.terms.end(); it++)
      disp_content(*it);
      std::cout <<expr.expr <<"\n";
  }
}
...
std::string insert_bracket(std::string str){
  ...
  for (std::string::iterator it=  str.begin(); it!=str.end();it++){
    ...
    if((*it)== ')'&& inserted)
    b_lvl--;
    if(std::string::npos != all_operators.find(*it) && inserted && b_lvl == 0){
      str.insert(it,')');
      finish = true;
      break;
    }//is operator
